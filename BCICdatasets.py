...
import scalp
import scipy
from mne_ext.signalproc import mne_apply
from collections import OrderedDict
from expriment.signalproc import exponential_running_standardize,bandpass_cnt
...
class BCICompetition4_2a(EEG_database):
    """
    datasets:BCI Competition IV 2a
    see   for more information
    """
    def __init__(self,path=r'G:\undergraduate\MIdatabase\BCIC4\2\a'):
        super(BCICompetition4_2a, self).__init__(name='BCIC42a',n_subject=9)
        self.path = path
        self.eventDescription = {'276': 'eyesOpen', '277': 'eyesClosed', '768': 'startTrail', '769': 'cueOnsetLeft',
                                 '770': 'cueOnsetRight'
            , '771': 'cueOnsetFoot', '772': 'cueOnsetTongue', '783': 'cueUnknown', '1023': 'rejectedTrial',
        ...
        self.eventDescription_e = {'1023': 'rejectedTrial','1072': 'eyeMovements','276': 'eyesOpen', '277': 'eyesClosed',
                                   '32766': 'startOfNewRun','768': 'startTrail','783': 'cueUnknown'}
        self.classes = ['cueOnsetLeft','cueOnsetRight','cueOnsetTongue','cueOnsetFoot']
        self.class_dict={0:'cueOnsetLeft',1:'cueOnsetRight',3:'cueOnsetTongue',2:'cueOnsetFoot'}
        self.class_dict_t= {'cueOnsetLeft':0,'cueOnsetRight':1, 'cueOnsetTongue':3, 'cueOnsetFoot':2}
        self.fs = 250
        self.names = ['EEG-Fz','EEG-0','EEG-1','EEG-2','EEG-3','EEG-4','EEG-5','EEG-C3', 'EEG-6', 'EEG-Cz','EEG-7', 'EEG-C4','EEG-8',
                      'EEG-9','EEG-10','EEG-11','EEG-12','EEG-13', 'EEG-14','EEG-Pz', 'EEG-15','EEG-16']
        self.C3 = 7
    ...
    def load_data(self,*args):
        super(BCICompetition4_2a, self).load_data(*args)
        for i in range(self.n_subject):
            "operate on self.subjects_data[i] "
            path_s = os.path.join(self.path,'s'+str(i+1))
            for file_path in os.listdir(path_s):
                ...
                if file_path.split('.')[-1] == 'gdf' and file_path.find('E') !=-1:
                    path_s_e = os.path.join(path_s,file_path)
            raw_t = mne.io.read_raw_gdf(path_s_t,stim_channel='auto')
            raw_t = self.preprocess_cnt(raw_t)
            event_t, _t = raw_t.info["gdf_events"]
            ...
            epochs_t = mne.Epochs(raw_t, event_t, event_id, event_repeated='merge',tmin=-0.5, tmax=4)
    ...
    def preprocess_cnt(self,raw_t):
        # path_s = os.path.join(self.path, 's' + str(subject_id + 1))
        # for file_path in os.listdir(path_s):
        #     if file_path.split('.')[-1] == 'gdf' and file_path.find('T') != -1:
        #         path_s_t = os.path.join(path_s, file_path)
        #     if file_path.split('.')[-1] == 'gdf' and file_path.find('E') != -1:
        ...
        #replace very big number nan with means of the channel without nan values
        ...
        #recreate mne.io.RawArray
        gdf_events = mne.events_from_annotations(raw_t)
        raw_t = mne.io.RawArray(data, raw_t.info, verbose="WARNING")
        # remember gdf events
        raw_t.info["gdf_events"] = gdf_events

        #extract events
        events, name_to_code = raw_t.info["gdf_events"]
        #name_to_code:{'1023': 1,'1072': 2,'276': 3,'277': 4,'32766': 5, '768': 6,'769': 7,'770': 8,'771': 9,'772': 10}
        trial_codes = [7,8,9,10]
        num_to_detract = 7
        code_start = 6
        if len(name_to_code) == 8:
            trial_codes = [5,6,7,8]
            num_to_detract = 5
            code_start = 4
        ...
        raw_t.info['events'] = trial_events
        raw_t.info['artifact_trial_mask'] = artifact_trial_mask

        #drop EOG channels
        raw_t.drop_channels(['EOG-left', 'EOG-central', 'EOG-right'])
        assert len(raw_t.ch_names) == 22 , 'channel dropping error'

        #become uv and bandpass,exp_standardize
        raw_t = mne_apply(lambda a: a * 1e6, raw_t)
        low_cut_hz = 4
        high_cut_hz = 38
        factor_new = 1e-3
    ...
    def create_signal_and_target(self,subject_id):
        sub_db = Subject_dataset(self,subject_id)
        x,y = next(iter(DataLoader(sub_db,len(sub_db))))
        return SignalAndTarget(x.numpy(),y.numpy().astype(np.int64))

# db = BCICompetition4_2a()
# db.load_data()
# dataset = db.create_signal_and_target(0)
#(22,1026)
#4 class
